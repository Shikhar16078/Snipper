        ...
        <div className={`absolute right-0 top-full mt-1.5 bg-panel border border-border rounded-xl shadow-2xl z-50 overflow-hidden animate-pop transition-all app-no-drag ${menuView === 'themes' ? 'w-44' : menuView === 'purge' ? 'w-52' : 'w-[256px]'}`}>
          {menuView === 'main' ? (
            <>
              ...
              {/* Toolbar position */}
              <div className="flex items-center justify-between px-3 py-2 mx-1">
                <p className="text-xs text-fg-2 font-medium">Toolbar</p>
                <div className="flex items-center gap-0.5 bg-fg/6 rounded-lg p-0.5">
                  {(['left', 'top', 'right', 'bottom'] as const).map((pos) => {
                    const icons: Record<string, React.ReactNode> = {
                      left:   <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 19l-7-7 7-7M4 12h16" />,
                      top:    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 11l7-7 7 7M12 4v16" />,
                      right:  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 5l7 7-7 7M20 12H4" />,
                      bottom: <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 13l-7 7-7-7M12 20V4" />,
                    }
                    const active = state.toolbarPosition === pos
                    return (
                      <button
                        key={pos}
                        onClick={() => dispatch({ type: 'SET_TOOLBAR_POSITION', payload: pos })}
                        title={pos.charAt(0).toUpperCase() + pos.slice(1)}
                        className={`p-1.5 rounded-md transition-colors ${active ? 'bg-panel text-fg shadow-sm' : 'text-muted hover:text-fg-2'}`}
                      >
                        <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          {icons[pos]}
                        </svg>
              ...
              <div className="px-1 pb-0.5">
                <div className="flex items-center gap-0.5">
                  <button
                    ...
                    className="flex-1 flex items-center justify-between px-3 py-2 text-xs text-fg-2 hover:text-fg hover:bg-fg/5 rounded-lg transition-colors group disabled:opacity-45 disabled:cursor-not-allowed"
                  >
